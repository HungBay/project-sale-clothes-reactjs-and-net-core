...
ListOrderShipping.propTypes = {};

function ListOrderShipping(props) {
  const { orders } = props;
  console.log(STATUS_ORDER);
  return (
    <div className="tab-pane fade" id="orderShipping">
      {orders
        ? orders
            .filter(orderDate => orderDate.statusOrder === STATUS_ORDER.shipping)
            .map((order, index) => (
